...
var home = process.env.INSIGHT_DB || (getUserHome() + '/pandacoin-insight');
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\Pandacoin\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/Pandacoin/';
  if (isLinux) dataDir = process.env.HOME + '/.pandacoin/';
}
...
var bitcoindConf = {
  ...
  user: process.env.BITCOIND_USER || 'pandauser',
  pass: process.env.BITCOIND_PASS || 'pandapass',
